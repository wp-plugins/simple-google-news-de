 ...
 * Description: Binde mit diesem einfachen Plugin den Google News Stream zu einem bestimmten Thema in die Sidebar, Artikel oder Seiten ein. 
 * Version: 1.0
...
function build_feed($atts, $newsUrl, $iswidget) {
	...
	if($atts['query'] != '') {
		$output .= '<p class="googleattribution">News via Google. <a rel="nofollow" href="' . $newsUrl . '">Noch mehr News zum Thema \'' . str_replace("+", " ", $atts['query']) . '\'</a></p>';
	}
	else {
		$output .= '<p class="googleattribution">News via Google. <a rel="nofollow" href="' . $newsUrl . '">Noch mehr News entdecken</a></p>';
	}
